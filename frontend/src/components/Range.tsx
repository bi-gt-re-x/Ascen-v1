 ...
 *
 * Painted with `--peak-snow` rather than the card's own colour, and that is the
 * one place the two have to differ. On white, the rock is a pale tint and the
 * card colour *is* snow. On the dark theme the rock is a tint over near-black,
 * which makes it lighter than the card — so a cap in the card's colour came
 * out as a dark notch cut into the summit, which is a hole rather than snow.
 */
...
function grow(name: string): Layer[] {
  ...
  return LAYERS.map((band, depth) => {
    ...
    return {
      d: ridge(points),
      caps: highest.map((peak, n) => cap(peak, 12 + depth * 3 - n * 2)),
    };
  });
}
