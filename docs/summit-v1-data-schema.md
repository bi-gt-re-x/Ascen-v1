---
name: summit-v1-data-schema
description: datastore tables + record counts for Summit v1 (SQLite data/summit.db, seeded from data/sql/*.sql)
metadata: 
  node_type: memory
  type: project
...
**Since 2026-07-28 the live datastore is a real SQLite database, `data/summit.db`** (git-ignored), built on first use from `data/sql/*.sql` — each of those holds its tables' CREATE TABLE plus the seed rows as INSERTs, under `-- ---- rows: <table> ----` markers. They are read only when the db is absent; after that the db is the only copy. `data/backups/*.json` is the old JSON store, kept as a backup. The root `database.db` is a different, dead file that nothing opens. See [[summit-v1-file-tree]], [[summit-v1-overview]]. Row counts as migrated 2026-07-28: users 6, tasks 714, goals 10, xp_events 234, focus_days 9, day_focus_notes 1, calendar_events 7, event_colors 21, metric_snapshots 36. Schemas (captured 2026-07-16, updated 2026-07-28):

- **users** (users.sql) — `id, username, password_hash` (pbkdf2 only), `xp, level, current_streak, best_streak, charge, tasks_completed, last_task_date, day_state, theme, created_at`, and (added 2026-07-22) optional `focus_history`: `{iso_date: {seconds, goal_hours}}` synced from the dashboard Focus panel via POST /api/focus_sync. The old `focus_history`/`day_focus` blobs now live as their own tables in focus.sql (`focus_days`: user_id/date/seconds/goal_hours, `day_focus_notes`: user_id/date/text). Account fields include those added by the 2026-07-26 accounts flow: `name, email, provider, email_verified, verify_token, profile_complete, daily_goal`. 6 demo users: `demo`, `riley`, `avery`, `casey`, `jordan`, `morgan` — all with an empty `password_hash`, so none of them can be signed into. See the note above the rows in users.sql.
