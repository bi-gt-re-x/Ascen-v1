...
EDITABLE = ('title', 'description', 'status', 'progress', 'goal_type',
...
def add_goal(body: AddGoal):
    ...
    if body.goal_type in targets and not targets[body.goal_type]:
        return fail(missing[body.goal_type])

    goals = db.goals()
    goals.append({
        "id": body.id or db.new_id('goals'),
        "user_id": body.username,
        "title": body.title,
        "description": body.description,
        "progress": 0,
        "target_value": targets.get(body.goal_type, targets['xp']),
        ...
        "focus_baseline_seconds": (focus_tracking.total_seconds(body.username)
                                   if body.goal_type == 'focus' else 0),
    ...
    db.save_goals(goals)
    return ok(message='Goal added successfully')
